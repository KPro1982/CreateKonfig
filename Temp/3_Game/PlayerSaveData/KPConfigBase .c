class PlayerSaveData<Class T>
{
	// the root should reflect your mod root folder. This is tytically your mod name
	static string Root = "$profile:\\Tester\\";
	protected string m_currentfilename;
	ref T m_data;
	
	void PlayerSaveData(string filename, ref T data)
	{
		auto trace = CF_Trace_2("PlayerSaveData", "PlayerSaveData").Add(filename).Add(data);
		...
		if(GetGame().IsDedicatedServer())
		{
			if(!OpenFile(m_currentfilename)) // attempt to load it
			{
				...
				CF_Log.Info("[PlayerSaveData::Constructor] no file found, creating file");
				UpdateClient();
				CF_Log.Info("[PlayerSaveData::Constructor] updating client with loaded data.");
			}
			else
			{
				CF_Log.Info("[PlayerSaveData::Constructor] file found and loaded");
			}
	...
	void ~PlayerSaveData() 
    {	
		auto trace = CF_Trace_0("PlayerSaveData", "~PlayerSaveData");
	}
	...
	void Init()
	{
		auto trace = CF_Trace_0("PlayerSaveData", "Init");
		...
		if(GetGame().IsDedicatedServer())
		{
			...
			GetRPCManager().AddRPC( "Tester", "XXX_SendCommandToServer", this, SingeplayerExecutionType.Both );
		}
		else
		{
			GetRPCManager().AddRPC( "Tester", "XXX_SendDataToClient", this, SingeplayerExecutionType.Both );		
		}
	...
	void SetData(T data)
	{
		auto trace = CF_Trace_1("PlayerSaveData", "SetData").Add(data);
		m_data = data;
	}
	
	ref T GetData()
	{
		auto trace = CF_Trace_0("PlayerSaveData", "GetData");
		return m_data;
	}
	...
	{
		auto trace = CF_Trace_3("PlayerSaveData", "Process").Add(savedata).Add(command).Add(filename);
		if(GetGame().IsDedicatedServer())
		{
	...
	{
		auto trace = CF_Trace_1("PlayerSaveData", "SaveFile").Add(filename);
		ref T k_data;
	...
	{
		auto trace = CF_Trace_1("PlayerSaveData", "NewFile").Add(filename);
		if(GetGame().IsDedicatedServer())
		{			
	 ...
	 bool OpenFile(string filename){
		auto trace = CF_Trace_1("PlayerSaveData", "Open").Add(filename);
	...
	void SendCommandToServer(ref T sendData, string command, string commandparam)
	{
		...
		GetRPCManager().SendRPC("Tester", "XXX_SendCommandToServer", new Param3< ref T, string, string >( sendData, command, commandparam ), true, NULL);
	}
	
	// runs on the server
	void UpdateServer(T savedata = null)
	{
		auto trace = CF_Trace_1("PlayerSaveData", "UpdateServer").Add(savedata);
		if(GetGame().IsDedicatedServer())
		{
			CopyData(savedata);
			Print("[PlayerSaveData::UpdateServer()] data: ");
			SaveFile(m_currentfilename);
		...
		{		
			...
			GetRPCManager().SendRPC("Tester", "XXX_SendCommandToServer", new Param3< ref T, string, string >( m_data, "updateserver", "" ), true, NULL);
		}	
	...
	void UpdateClient()
	{
		auto trace = CF_Trace_0("PlayerSaveData", "UpdateClient");
		if(GetGame().IsDedicatedServer())
		{
			GetRPCManager().SendRPC("Tester", "XXX_SendDataToClient", new Param1< ref T >( m_data ), true, NULL);	

		}
		else
		{
			GetRPCManager().SendRPC("Tester", "XXX_SendCommandToServer", new Param3< ref T, string, string >( null, "updateclient", "" ), true, NULL);
		}
	...
	{		
		if(GetGame().IsDedicatedServer())
		{
			auto trace = CF_Trace_4("PlayerSaveData", "XXX_SendCommandToServer").Add(type).Add(ctx).Add(sender).Add(target);
	...
	void XXX_SendDataToClient( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target ) {
		if(!GetGame().IsDedicatedServer())
		{
			auto trace = CF_Trace_4("PlayerSaveData", "XXX_SendDataToClient").Add(type).Add(ctx).Add(sender).Add(target);;	
			Param1< ref T > data;
			if ( !ctx.Read( data ) ) return;
